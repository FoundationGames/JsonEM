...
public class JsonEMCodecs {
        private static final Codec<Vector3f> VECTOR3F_CODEC = Codec.FLOAT.listOf().comapFlatMap(
                        coordinates -> Util.toArray(coordinates, 3)
                                        .map(coords -> new Vector3f(coords.get(0), coords.get(1),
                                                        coords.get(2))),
                        vec -> List.of(Float.valueOf(vec.x), Float.valueOf(vec.y),
                                        Float.valueOf(vec.z)));
    ...
    public static final Codec<Vector2f> VECTOR2F = Codec.FLOAT.listOf().comapFlatMap((vec) ->
        ...
        private static ModelCuboidData createCuboidData(Optional<String> name, Vector3f offset, Vector3f dimensions,
                        Dilation dilation, boolean mirror, Vector2f uv, Vector2f uvSize) {
                return ModelCuboidDataAccess.jsonem$create(name.orElse(null), uv.getX(), uv.getY(), offset.x, offset.y,
                                offset.z, dimensions.x, dimensions.y, dimensions.z, dilation, mirror, uvSize.getX(),
                                uvSize.getY());
        }
    ...
    public static final Codec<ModelCuboidData> MODEL_CUBOID_DATA = RecordCodecBuilder.create((instance) ->
            instance.group(
                    ...
                    VECTOR3F_CODEC.fieldOf("offset").forGetter(obj -> ((ModelCuboidDataAccess)(Object)obj).jsonem$offset()),
                    VECTOR3F_CODEC.fieldOf("dimensions").forGetter(obj -> ((ModelCuboidDataAccess)(Object)obj).jsonem$dimensions()),
        ...
        private static Codec<ModelPartData> createPartDataCodec() {
                return RecordCodecBuilder.create((instance) -> instance.group(
                                MODEL_TRANSFORM.optionalFieldOf("transform", ModelTransform.NONE)
                                                .forGetter(obj -> ((ModelPartDataAccess) obj).jsonem$transform()),
                                Codec.list(MODEL_CUBOID_DATA).fieldOf("cuboids")
                                                .forGetter(obj -> ((ModelPartDataAccess) obj).jsonem$cuboids()),
                                LazyTypeUnboundedMapCodec.of(Codec.STRING, JsonEMCodecs::createPartDataCodec)
                                                .optionalFieldOf("children", new HashMap<>())
                                                .forGetter(obj -> ((ModelPartDataAccess) obj).jsonem$children()))
        ...
        public static final Codec<TexturedModelData> TEXTURED_MODEL_DATA = RecordCodecBuilder
                        .create((instance) -> instance.group(
                                        TEXTURE_DIMENSIONS.fieldOf("texture").forGetter(
                                                        obj -> ((TexturedModelDataAccess) obj).jsonem$texture()),
                                        Codec.unboundedMap(Codec.STRING, MODEL_PART_DATA).fieldOf("bones").forGetter(
                                                        obj -> ((ModelPartDataAccess) ((TexturedModelDataAccess) obj)
                                                                        .jsonem$root().getRoot()).jsonem$children()))
